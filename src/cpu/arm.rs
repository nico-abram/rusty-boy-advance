...
fn SDT(cpu: &mut Cpu, opcode: u32) {
  ...
  let mut addr = ((cpu.regs[rn] as i64)
    + if is_up {
      offset as i64
    } else {
      -(offset as i64)
    ...
    let (op2, shift_carry) = if imm_shift_zero {
      ...
      match shift_type {
        0 => (
          ...
          Some(
            (register_value & (1 << std::cmp::max(shift_amount - 1, 0))) != 0 || shift_amount == 0,
          ),
        ),
        1 => (
  ...
  match psr_opcode as u8 {
    ...
    4 => {
      ...
      *res = result;
      set_all_flags(
        *res,
        Some(addition_carries(result, rn, op2)),
        Some(overflow),
      );
    }
    5 => {
...
fn MSR(cpu: &mut Cpu, opcode: u32) {
  ...
  let valid_bits_mask: u32 =
    (if control_fields { STATE_MASK } else { 0 }) | (if control_flags { FLAGS_MASK } else { 0 });
  ...
  *dest = CPSR((old & invalid_bits_mask) | (val & valid_bits_mask));
  if cpu.cpsr.mode() != CpuMode::User && control_fields {
    cpu.set_mode(CpuMode::from_byte(
      ((operand & 0x0000000F) | 0x00000010) as u8,
    ));
...
fn MRS(cpu: &mut Cpu, opcode: u32) {
  ...
  cpu.regs[((opcode & 0x0000_F000) >> 12) as usize] = if use_mode_cpsr {
    cpu.cpsr.0
  } else {
    cpu.spsr[cpu.cpsr.mode().as_usize()].0
  };
}
